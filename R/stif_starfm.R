...
#' situation a single pair of images is used to predict the fine scale image
#' at t_2.
#' 
...
stif_starfm <- function(c1,
  ...
  out[] <- starfm_spatial_filter_fast(c1 = c1.intp[], c2 = c2.intp[], f1 = f1[],
                                       dims = dim(f1), redb = 1, nirb = 4, w0, ns)
